...
module.exports = function (document) {
  var p = $private(document);
  if (!p.modifier) {
    p.modifier = new Wrapper(document);
  }
  return p.modifier;
};
...
function getCurrentRelations(modifier) {
  return Object.keys(modifier.relations).reduce(function (o, key) {
...
Wrapper.prototype.init = function (values, schema) {
  ...
  this.relations = schema.relations.reduce(function (o, relation) {
    ...
    if (relation.type === Type.HasMany) {
      o[relation.name] = Array.isArray(value)
        ? [].map.call(value, function (subrow) {
...
Wrapper.prototype.getChanges = function () {
  if (!this.fields) {
    throw new Error("Undefined state: " + JSON.stringify(this.document));
  }
