...
SequelizeRepository.prototype.findWhere = function (q, opts) {
  ...
  if (opts.attributes) {
    params.attributes = opts.attributes.slice(0);
    var primaryKeyAttribute = this.schema.primaryField.name;
    if (params.attributes.indexOf(primaryKeyAttribute) === -1) {
      params.attributes.push(primaryKeyAttribute);
    }
    params.include = [];
  }
  var relationqs = {};
...
Repository.prototype.stream = function (q, opts) {
  ...
  options.attributes = opts.attributes || this.schema.fields.map(function (field) {
    return field.name;
  });
