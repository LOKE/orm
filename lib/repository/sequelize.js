...
function sequelizeType(field) {
  ...
  if (type === Type.Date) return DATE;
  if (type === Type.Boolean) return BOOLEAN;
  if (type === Type.Text) return TEXT;
  if (type === Type.Decimal) return DECIMAL;
  if (type === Type.Id) return INTEGER.UNSIGNED;
  if (type === Type.Enum) return new ENUM(field.values);
}
...
function makeQuery(repo, q, opts) {
  ...
  // According to the docs Utils is deprecated. We should try and move away from needing this.
  sequelize.Sequelize.Utils.mapOptionFieldNames(options, model);
...
Repository.prototype._stream = function (sql, replacements, opts, ctor) {
  ...
  connectionManager
    ...
    .then(
      /**
       * @param {import("mysql2").Connection | import("pg").Client} connection
       */
      function (connection) {
        const sqlStream =
          connection instanceof Client
            ? connection.query(new QueryStream(sql, replacements))
            : connection
...
SequelizeRepository.prototype.literal = function (sqlCode) {
  var sequelize = $private(this).sequelize;
  return sequelize.literal(sqlCode);
};
