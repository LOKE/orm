...
function makeAttributes(schema, options) {
  ...
  var primaryKeyAttribute = schema.primaryField.name;
  if (!options.raw && primaryKeyAttribute && attributes.indexOf(primaryKeyAttribute) === -1) {
    attributes.push(primaryKeyAttribute);
  }
  return attributes
}
