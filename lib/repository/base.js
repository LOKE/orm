...
Repository.prototype.findById = function (id, opts) {
  ...
  if (keys.length !== 1) throw new Error('This repository has ' + keys.length + ' primary keys. To use .findById(...) it must have exactly one.');
...
Repository.prototype.persist = function (document, options, parent, parentRepo, parentRelation) {
  ...
  return Promise.resolve(modifier.persisting)
  ...
  .then(function () {
    ...
    return modifier.persisting = repo.insert(changes, {transaction: transaction}, parent, parentRepo, parentRelation)
  ...
  .then(function () {
    return Promise.all(schema.relations.map(function (relation) {
      ...
      if (relation.type === Type.HasMany) {
        ...
        var missingFromArray = oldValue && oldValue.filter(function (oldDoc) {
          if (~value.indexOf(oldDoc)) return false;
          if (value.some(function (newDoc) {
            return isSameDocument(schema, oldDoc, newDoc);
          })) return false;
          return true;
        });
        return Promise.all(value.map(function (subdoc) {
          return relation.target.persist(subdoc, {transaction: transaction}, document, repo, relation, transaction)
          .then(function () {
        ...
        .then(function () {
          if (oldValue && (oldValue !== value) && !isSameDocument(schema, oldValue, value)) {
        ...
        .then(function () {
          if (value) return relation.target.persist(value, {transaction: transaction}, document, repo, relation, transaction);
        })
        .then(function () {
...
Repository.prototype.didApplyChanges = function (document, fieldValues, relationValues) {
